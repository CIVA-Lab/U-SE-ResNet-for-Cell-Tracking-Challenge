...
parser.add_argument('--is_train_all', type=bool, nargs='?', const=True, default=False,
                    help='Single model for all gt data.')

args = parser.parse_args()

dataset = args.dataset
# getting train data from CSV file for gt
csv_path = '../../Data/train/CSV/' + dataset + '.csv'
...
train_df, val_df = train_test_split(data_df, test_size=.1, random_state=42)
...
training_id = f"gt_weighted_loss_{dataset}_unet_{train_resolution[0]}_{train_resolution[1]}_{backbone}_{pretrained if pretrained else 'nonpretrained'}_{date_time}"
...
print('Loading data...')
# 3D datasets are converted to 2D to train gt model
trainset = CTC2DDatasetWithMarkersSW(
        train_df, erosion=args.erosion, resolution=train_resolution, resize_to=args.resize_to)
valset = CTC2DDatasetWithMarkersSW(
...
for i in tqdm(range(epochs)):
    ...
    if max_score < valid_logs['iou_score']:
        ...
        if args.is_train_all:
            torch.save(model, f'{save_dir}/gt_all_best_model.pth')
            torch.save(model, f'../trained_models/gt_all_best_model.pth')
        else:
            torch.save(model, f'{save_dir}/gt_sw{dataset}_best_model.pth')
            torch.save(model, f'../trained_models/gt_sw_{dataset}_best_model.pth')
