...
def main():
    ...
    try:
        model = torch.load(f'../trained_models/gt_sw_{args.dataset}_best_model.pth', map_location=args.device)
    ...
    for path in paths:      
        ...
        labeled_mask = postprocess_mask_and_markers_3d(mask, markers, area_thresh=args.area_threshold)

        labeled_mask = labeled_mask.astype('uint16')

        outname = path.split('/')[-1]
        outdir = '/'.join(path.split('/')[:-1]) + '_RES-GT'
